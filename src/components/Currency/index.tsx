...
const Currency:FC<objProps> = ({setQuant1, setQuant2, setChanges, setVal}) => {
    ...
    const countr = localStorage.getItem("currencyFrom")? JSON.parse(localStorage.getItem("currencyFrom") as string).toString() :"EUR";
    const countr1 = localStorage.getItem("currencyTo")? JSON.parse(localStorage.getItem("currencyTo") as string).toString()  :"USD";
    ...
    useEffect(() => {
        let ind= 0;
    !exchange ?
    ind = (currencyKeys && currencyKeys.findIndex((el:any) => el === valuesTo)):
    ind = (currencyKeys && currencyKeys.findIndex((el:any) => el === valuesTo));    
    if (ind !== -1) {
        setCountry(ind);
    }
    ...
    const symbole1:qauntProps | undefined = data.find(el => el.currency?.code == valuesFrom);
    const symbole2:qauntProps | undefined = data.find(el => el.currency?.code == valuesTo);
    ...
    useEffect(() => {
        ...
        fetch(`https://api.fastforex.io/fetch-all?from=${valuesFrom}&api_key=a58c7e9b7a-988376f8c5-sc54ik`)
            .then(res => res.json())
            .then(data => {
                setConvert(data);       
    ...
    <div className="hero__currency">
              ...
              <form onChange={handleSubmit} className="currency__exchange">
                ...
                <div className="currency__input currency__input--Amount">
                    <label htmlFor="Amount">Amount {loader && <CircularProgress size="1.2rem"/>}</label>
                    <input defaultValue={quantity === 0 ? "" : quantity} ref={amount} type="number" placeholder={!exchange ? `${symbole1?.currency.symbol ? symbole1?.currency.symbol : symbole2?.currency.code}`:"$"}/>
                </div>
                        ...
                        <span>From</span> 
                       {
                        !exchange ? 
                            data && data.map((el, index) => {
                                return el.currency.code == valuesFrom && <img key={index} width={32} src={el.flag ? el.flag : euro} alt="flag icon" />
                            }) :
                            data && data.map((el, index) => {
                                return el.currency.code == valuesTo && <img key={index} width={32} src={el.flag ? el.flag : euro} alt="flag icon" />
                            })
                       }
                    </label>
                       {
              ...
              <div className="currency__priceChange">
                <h5>{`${quantity}`}.00 {!exchange ? (data && data.map(el => (valuesFrom == el.currency.code && el.currency.name))):(data && data.map(el => (valuesTo == el.currency.code && el.currency.name)))} =</h5>
                <h1>{whole}<span className="faded-digits">{decimalPart ? decimalPart : "00"}</span> {exchange ? (data && data.map(el => (valuesFrom == el.currency.code && el.currency.name))):(data && data.map(el => (valuesTo == el.currency.code && el.currency.name)))}</h1>
                 {
                    exchange ?
                    <div>
                    <p>1 {valuesTo} = {qiymat} {valuesFrom}</p>
                    <p>1 {valuesFrom} = {1 / qiymat} {valuesTo}</p>
                </div>:
                <div>
                    <p>1 {valuesFrom} = {1 / qiymat} {valuesTo}</p>
                    <p>1 {valuesTo} = {qiymat} {valuesFrom}</p>
